...
Outlet::Outlet(Module* _parent, int _x, int _y)
{
    parent = _parent;
    x = _x;
    y = _y;
...
void Outlet::push_sample(double q){
    if(!connection) return;
    else connection->in_sample = q;
}

bool Outlet::is_point_within(double _x, double _y){
    _x -= parent->xoffset + x;
    _y -= y;
    _x = _x*_x;
    _y = _y*_y;
...
void Outlet::draw(const Cairo::RefPtr<Cairo::Context>& cr){
    ...
    //hole
    cr->set_source_rgb(60.0/256.0, 20.0/256.0, 20.0/256.0);
